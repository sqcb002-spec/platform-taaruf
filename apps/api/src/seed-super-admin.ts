...
async function seedSuperAdmin() {
  let [user] = await db.select({ id: users.id, role: users.role, emailVerified: users.emailVerified, status: users.status }).from(users).where(eq(users.email, email)).limit(1);
  let created = false;
  ...
  if (!user) {
    ...
    [user] = await db.select({ id: users.id, role: users.role, emailVerified: users.emailVerified, status: users.status }).from(users).where(eq(users.email, email)).limit(1);
    created = true;
  }
  ...
  const outcome = created ? "dibuat; tautan pengaturan kata sandi telah dikirim" : needsUpdate ? "dipromosikan" : "sudah aktif; tidak ada perubahan";
  process.stdout.write(`Super admin ${outcome}: ${email}.\n`);
}
