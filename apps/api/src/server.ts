...
const profileCoreSchema = z.object({
  fullName: z.string().trim().min(3).max(120), gender: z.enum(["Ikhwan", "Akhwat"]), birthDate: z.coerce.date(), phone: z.string().trim().regex(/^\+?[0-9]{10,15}$/), province: z.string().trim().min(2).max(80), city: z.string().trim().min(2).max(80), district: z.string().trim().min(2).max(80), village: z.string().trim().min(2).max(80), maritalStatus: z.string().min(1).max(60), occupation: z.string().trim().min(3).max(500), salaryRange: z.string().min(1).max(80), educationLevel: z.string().min(1).max(80), quranReading: z.string().min(1).max(80), quranMemorization: z.string().min(1).max(80), prayer: z.string().min(1).max(120), studyFrequency: z.string().min(1).max(80), music: z.string().min(1).max(80), smoking: z.string().min(1).max(40), widowMarriage: z.string().min(1).max(40), manhaj: z.string().trim().min(2).max(120).optional(), ethnicity: z.string().trim().min(2).max(80).optional(), heightCm: z.coerce.number().int().min(120).max(230).optional(), weightKg: z.coerce.number().int().min(30).max(250).optional(),
});
